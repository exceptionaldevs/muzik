...
public class ColorUtils {
    ...
    public static @ColorInt int generateDarkerOrLighterColor(int color) {
        float[] hsl = new float[3];
        android.support.v4.graphics.ColorUtils.colorToHSL(color, hsl);
        boolean isDark = com.exceptionaldevs.muzyka.utils.ColorUtils.isDark(hsl);
        hsl[2] = isDark ?
                Utils.constrainFloatInRange(hsl[2] + 0.2f, 0, 1) : Utils.constrainFloatInRange(hsl[2] - 0.2f, 0, 1);
        return android.support.v4.graphics.ColorUtils.HSLToColor(hsl);
    }
