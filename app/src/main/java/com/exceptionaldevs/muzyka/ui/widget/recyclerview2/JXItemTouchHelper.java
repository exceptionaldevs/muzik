 ...
 * {@link JXItemTouchHelper.Callback} class or implementing {@link JXItemTouchHelper.ViewDropHandler}
 ...
 */
public class JXItemTouchHelper extends RecyclerView.ItemDecoration implements RecyclerView.OnChildAttachStateChangeListener {
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                if (mSelected == null) {
                    ...
                    if (animation != null) {
                        ...
                        if (mPendingCleanup.remove(getSwipeView(animation.mViewHolder))) {
     ...
     */
    public JXItemTouchHelper(Callback callback) {
        mCallback = callback;
    }
    ...
    public View getSwipeView(ViewHolder viewHolder) {
        if (viewHolder == null)
            return null;
        if (mCallback.hasDragFlag(mRecyclerView, viewHolder))
            return viewHolder.itemView;
        if (viewHolder.itemView instanceof ChildArtistItem)
            return ((ChildArtistItem) viewHolder.itemView).getChildArtist();
        return viewHolder.itemView;
    }
    ...
    private void getSelectedDxDy(float[] outPosition) {
        if ((mSelectedFlags & (LEFT | RIGHT)) != 0) {
            outPosition[0] = mSelectedStartX + mDx - getSwipeView(mSelected).getLeft();
        } else {
            outPosition[0] = ViewCompat.getTranslationX(getSwipeView(mSelected));
        }
        if ((mSelectedFlags & (UP | DOWN)) != 0) {
            outPosition[1] = mSelectedStartY + mDy - getSwipeView(mSelected).getTop();
        } else {
            outPosition[1] = ViewCompat.getTranslationY(getSwipeView(mSelected));
        }
    }
    ...
    /**
     * Starts dragging or swiping the given View. Call with null if you want to clearQueue it.
     *
    ...
    private void select(ViewHolder selected, int actionState) {
        ...
        if (actionState == ACTION_STATE_DRAG) {
            ...
            mOverdrawChild = getSwipeView(mSelected);
        ...
        if (mSelected != null) {
            ...
            if (getSwipeView(prevSelected).getParent() != null) {
                ...
                final RecoverAnimation rv = new RecoverAnimation(prevSelected, animationType, prevActionState, currentTranslateX, currentTranslateY, targetTranslateX, targetTranslateY) {
                    ...
                    public void onAnimationEnd(Animator animation) {
                        ...
                        if (swipeDir <= 0) {
                            ...
                            mPendingCleanup.add(getSwipeView(prevSelected));
                        ...
                        if (mOverdrawChild == getSwipeView(prevSelected)) {
                            removeChildDrawingOrderCallbackIfNecessary(getSwipeView(prevSelected));
                        }
                    }
                };
        ...
        if (selected != null) {
            ...
            mSelectedStartX = getSwipeView(selected).getLeft();
            mSelectedStartY = getSwipeView(selected).getTop();
            mSelected = selected;

            if (actionState == ACTION_STATE_DRAG) {
                getSwipeView(mSelected).performHapticFeedback(HapticFeedbackConstants.LONG_PRESS);
            }
        }
    ...
    public void onChildViewDetachedFromWindow(View view) {
        ...
        if (mSelected != null && holder == mSelected) {
            ...
            if (mPendingCleanup.remove(getSwipeView(holder))) {
    ...
    private View findChildView(MotionEvent event) {
        ...
        if (mSelected != null) {
            final View selectedView = getSwipeView(mSelected);
        ...
        for (int i = mRecoverAnimations.size() - 1; i >= 0; i--) {
            final RecoverAnimation anim = mRecoverAnimations.get(i);
            final View view = getSwipeView(anim.mViewHolder);
     ...
     * {@link JXItemTouchHelper.Callback#isLongPressDragEnabled()}.
     * <p/>
     ...
     * ItemTouchHelper
     * is attached.</li>
     * <li>{@link JXItemTouchHelper.Callback} must have dragging enabled.</li>
     ...
     *                   RecyclerView.
     * @see JXItemTouchHelper.Callback#isItemViewSwipeEnabled()
     */
     ...
     * behavior
     * by overriding {@link JXItemTouchHelper.Callback}
     * <p/>
     * For this method to work:
     * <ul>
     * <li>The provided ViewHolder must be a child of the RecyclerView to which this
     * ItemTouchHelper is attached.</li>
     * <li>{@link JXItemTouchHelper.Callback} must have swiping enabled.</li>
     * <li>There must be a previous touch event that was reported to the ItemTouchHelper
     * through RecyclerView's ItemTouchListener mechanism. As long as no other ItemTouchListener
     * grabs previous events, this should work as expected.</li>
     * </ul>
    ...
    private RecoverAnimation findAnimation(MotionEvent event) {
        ...
        for (int i = mRecoverAnimations.size() - 1; i >= 0; i--) {
            final RecoverAnimation anim = mRecoverAnimations.get(i);
            if (getSwipeView(anim.mViewHolder) == target) {
                return anim;
            }
        }
        return null;
     ...
     * {@link JXItemTouchHelper}.
     */
    public static interface ViewDropHandler {

        /**
         * Called by the {@link JXItemTouchHelper} after a View is dropped over another View.
         * <p/>
    ...
    public abstract static class Callback {
         ...
         *                    it is cleared.
         * @param actionState One of {@link JXItemTouchHelper#ACTION_STATE_IDLE},
         *                    {@link JXItemTouchHelper#ACTION_STATE_SWIPE} or
         *                    {@link JXItemTouchHelper#ACTION_STATE_DRAG}.
        ...
        private void onDraw(Canvas c, RecyclerView parent, ViewHolder selected,
            ...
            for (int i = 0; i < recoverAnimSize; i++) {
                final JXItemTouchHelper.RecoverAnimation anim = recoverAnimationList.get(i);
                anim.update();
         ...
         * <p/>
         * This is a good place to clearQueue all changes on the View that was done in
    ...
    public abstract static class SimpleCallback extends Callback {
        ...
        /**
         * Updates the default swipe directions. For example, you can use this method to togglePlayback
        ...
        /**
         * Updates the default drag directions. For example, you can use this method to togglePlayback
         * certain directions depending on your use case.
         *
    ...
    private class RecoverAnimation implements Animator.AnimatorListener  {
        ...
        public RecoverAnimation(ViewHolder viewHolder, int animationType, int actionState, float startDx, float startDy, float targetX, float targetY) {
            ...
            mValueAnimator.setTarget(getSwipeView(viewHolder));
